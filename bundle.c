...
void bundle_method(Problem *PP, double *t, int bdl_iter, double fixedValue) {
    ...
    extern double *biqbin_gamma;           // dual variable to cutting plane inequalities
    ...
    while ( bdl_cnt < bdl_iter ) {
        ...
        beta = -1.0;
        TRANS = 'T';
        dgemv_(&TRANS, &m, &k, &alpha, G, &m, biqbin_gamma, &inc, &beta, zeta, &inc);

        /*** solve QP ***/
        lambda_eta(PP, zeta, G, biqbin_gamma, dgamma, lambda, eta, t);

        /*** make a step: gamma_test = gamma + dgamma; ***/
        for (int i = 0; i < m; ++i)
            gamma_test[i] = biqbin_gamma[i] + dgamma[i];
        ...
        if (f - f_test > 0.05 * del) { // SERIOUS STEP
            ...
            // gamma = gamma_test
            dcopy_(&m, gamma_test, &inc, biqbin_gamma, &inc);  
