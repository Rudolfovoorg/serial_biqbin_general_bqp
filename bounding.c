...
extern double *biqbin_gamma;                        // dual multiplers for triangle inequalities
...
double SDPbound(BabNode *node, Problem *SP, Problem *PP) {
    ...
    viol3 = updateTriangleInequalities(PP, biqbin_gamma, &Tri_NumAdded, &Tri_NumSubtracted);
    ...
    while (!done) {
        ...
        if (!prune && !giveup) {
            ...
            {
                if ( params.include_Pent ) {
                    viol5 = updatePentagonalInequalities(PP, biqbin_gamma, &Pent_NumAdded, &Pent_NumSubtracted, triag);  
                }
                if ( params.include_Hepta ) {
                    if (penta == 0)
                        penta = PP->NPentIneq;
                    viol7 = updateHeptagonalInequalities(PP, biqbin_gamma, &Hepta_NumAdded, &Hepta_NumSubtracted, triag + penta);  
                }
            }
        ...
        if (!done) {
            ...
            for (int i = 0; i < PP->NPentIneq; ++i)
                biqbin_gamma[i + PP->NIneq] = Pent_Cuts[i].y;

            for (int i = 0; i < PP->NHeptaIneq; ++i)
                biqbin_gamma[i + PP->NIneq + PP->NPentIneq] = Hepta_Cuts[i].y;


            fct_eval(PP, biqbin_gamma, X_test, g);
